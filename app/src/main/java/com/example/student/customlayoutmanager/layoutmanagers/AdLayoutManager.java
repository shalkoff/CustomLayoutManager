package com.example.student.customlayoutmanager.layoutmanagers;
...
public class AdLayoutManager extends RecyclerView.LayoutManager
{
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state)
    {
        ...
        if (layoutInfoLookup.getRowSpan(firstPosition) == SpanCount.TWO)
        {
            firstPosition--;
        }
    ...
    {
        if (layoutInfoLookup.getRowSpan(position) == SpanCount.TWO)
        {
            // 1 column x 2 rows
            addTallView(recycler, position, scrollingDown);
        }
        else if (layoutInfoLookup.getColumnSpan(position) == SpanCount.TWO)
        {
            // 2 columns x 1 row
    ...
    private void updateColumnValues(final boolean updateTopValues)
    {
        ...
        for (int i = startIndex; i != endIndex; i += step)
        {
            ...
            if (layoutInfoLookup.getColumnSpan(adapterPosition) == SpanCount.TWO)
            {
                if (updateTopValues)
                {
